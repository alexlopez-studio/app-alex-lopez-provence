...
function formatEur(n: number) { return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(n) }
...
const relItem: CSSProperties    = { display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 0' }
const relLbl: CSSProperties     = { fontSize: '13px', fontWeight: 500, color: fg, flex: 1 }
const relSub: CSSProperties     = { fontSize: '11px', fontWeight: 400, color: muted }
const relInner: CSSProperties   = { flex: 1 }
const relEmoji: CSSProperties   = { fontSize: '20px' }
const actRow: CSSProperties     = { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }
const actEmoji: CSSProperties   = { fontSize: '18px' }
const actTitle: CSSProperties   = { fontSize: '13px', fontWeight: 600, color: fg }
const actDate: CSSProperties    = { fontSize: '11px', color: muted }
const actNote: CSSProperties    = { fontSize: '12px', fontWeight: 300, color: muted, lineHeight: 1.6 }
...
const fldLbl: CSSProperties     = { fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', color: muted } as CSSProperties
...
const STATUT: Record<Statut, { label: string; color: string; bg: string }> = {
...
function relBorder(last: boolean): CSSProperties {
  return { ...relItem, borderBottom: last ? 'none' : '1px solid ' + border }
}
      ...
      <div style={pageWrap}>
        ...
        <div style={twoCol}>
          <div style={card}>
            ...
            {RELANCES.map((r, i) => (
              <div key={i} style={relBorder(i === RELANCES.length - 1)}>
                <span style={relEmoji}>{r.emoji}</span>
                <div style={relInner}><div style={relLbl}>{r.label}</div><div style={relSub}>{r.sub}</div></div>
          ...
          <div style={card}>
            ...
            <div style={actRow}>
              <span style={actEmoji}>{'\ud83d\udcc5'}</span>
              <div><div style={actTitle}>{'Lead cr\u00e9\u00e9'}</div><div style={actDate}>{date}</div></div>
            </div>
            <div style={actNote}>{'Les ouvertures d\'emails et consultations de rapport appara\u00eetront ici.'}</div>
          </div>
        </div>
        {(adresse || surface) && (
          <div style={bienCard}>
            ...
            <div style={bienInner}>
              <div style={bienTitle}>{typeLbl}{surface ? ' \u00b7 ' + surface + ' m\u00b2' : ''}{nbPieces ? ' \u00b7 ' + nbPieces + ' pi\u00e8ces' : ''}</div>
